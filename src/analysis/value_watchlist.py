...
Fires when GEX regime, technicals (MA200, RSI), put wall proximity,
and earnings clearance align. Two-phase execution:
  Phase 1 (parallel): fetch all price histories + earnings dates
  Phase 2 (parallel): per-stock GEX computation + scoring
...
# ETFs have no meaningful per-stock GEX on Alpaca indicative feed
_ETF_TICKERS = {"VTV", "SCHD", "SPLG", "VOO", "SPY", "QQQ"}

STRONG_THRESHOLD = {"own_now": 6, "own_lower": 7}
...
def _scan_one(ticker: str, label: str,
    ...
    try:
        ...
        score, signal, reasons, blockers, earn_clear = _score_ticker(
...
def scan_watchlist(spy_regime: str = "UNKNOWN",
    ...
    # Phase 1: histories + earnings in parallel (2 batch calls)
